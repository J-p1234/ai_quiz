...
def start_quiz():
    ...
    restart_game = input('Do you want to play again? Answer (yes/no): ').strip()
    while restart_game not in ["yes", "no"]:  # perimenei mexri na apantiseis yes/no, den dexetai allou eidous apantisi
        restart_game = input("Answer (yes/no): ").strip()
    if restart_game == 'yes':
        restart_quiz()
